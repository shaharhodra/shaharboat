...
public class UI : MonoBehaviour {
	...
	private float opasity = 0.5f;
	private bool swshader = false;
	private bool sw = false;
	private GameObject go;
	private Color testcolor;
	private Color precolor = Color.black;
	private float prefloat00 = 0f;
	private float prefloat01 = 0f;
	...
	void OnGUI() {
		...
		if (GUI.Button(new Rect(245, 45, 100, 20),"Shader Switch")){
			sw = true;
		}
		if (GUI.Button(new Rect(350, 23, 150, 20),"Shader Forge Sample")){
		...
		if(swshader){
		GUI.Label(new Rect(25, 200, 100, 20), "Opasity");
		opasity = GUI.HorizontalScrollbar(new Rect(25, 220, 200, 30), opasity, 0.1F, 0.2F, 1.0F);
		}
	}
	
	void Start () {
		go = GameObject.Find ("Sea");
		...
		go.transform.GetComponent<MeshRenderer> ().material.shader = Shader.Find("Shader Forge/Examples/DeepSea_Opaqe");
	}
	...
	void Update () {
		...
		if (precolor != testcolor || prefloat00 != bulgeScale || prefloat01 != opasity){
			...
			go.transform.GetComponent<MeshRenderer> ().material.SetFloat ("_Opasity", opasity);
			precolor = testcolor;
		...
		if (sw) {
			if (swshader){
				...
				swshader = true;
				go.transform.GetComponent<MeshRenderer> ().material.shader = Shader.Find("Shader Forge/Examples/DeepSea_AlphaBlending");
			}
			sw = false;
		}
		else{return;}
